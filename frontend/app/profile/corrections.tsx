...
export default function MyCorrectionsScreen() {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    ...
    queryFn: () => correctionRequestsApi.listMine().then((r) => r.data),
    enabled: !!token,
  });
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{t("correction.my_list_title")}</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      {isLoading ? (
        ...
        <View style={styles.center}>
          ...
          <Text style={styles.emptyTitle}>
            {t("correction.my_list_load_error")}
          </Text>
          <TouchableOpacity
            ...
            disabled={isFetching}
          >
            <Text style={styles.retryText}>{t("correction.my_list_retry")}</Text>
          </TouchableOpacity>
        </View>
      ) : items.length === 0 ? (
        <View style={styles.center}>
          ...
          <Text style={styles.emptyTitle}>
            {t("correction.my_list_empty_title")}
          </Text>
          <Text style={styles.emptyBody}>
            {t("correction.my_list_empty_body")}
          </Text>
        </View>
      ) : (
        <FlatList
